...
from app.llm.ollama_adapter import OllamaLLM, OllamaAdapter as OllamaEmbedding
...
async def get_llama_documents_from_doc(
    doc: DocumentSchema,
    fs: Optional[AsyncFileSystem] = None,
) -> List[LlamaIndexDocument]:
    """
    从 DocumentSchema 获取 LlamaIndex Document
    """
    if fs is None:
    ...
    with TemporaryDirectory() as temp_dir:
        local_path = Path(temp_dir) / "doc.pdf"
        
        # 从 S3 下载文件
        s3_path = doc.url.replace(f"{settings.CDN_BASE_URL}/", "")
        ...
        reader = PDFReader()
        try:
            docs = reader.load_data(file=local_path)  # 使用 file 参数而不是直接传递路径
        except Exception as e:
            logger.error(f"Error reading PDF file: {e}")
            raise
        
        # 添加元数据
        for llama_doc in docs:
            llama_doc.metadata = {
                DB_DOC_ID_KEY: str(doc.id),
                **doc.metadata_map,
                "file_name": s3_path.split("/")[-1]  # 添加文件���到元数据
            }
...
async def build_doc_id_to_index_map(
    ...
    for doc in docs:
        try:
            ...
            if indices:
                logger.info(f"Found existing index for document {doc.id}")
                doc_id_to_index[str(doc.id)] = indices[0]
                continue
        except Exception as e:
            logger.warning(f"No existing index found for document {doc.id}: {e}")
        
        # 如果没有找到索引，创建新的
        try:
            llama_docs = await get_llama_documents_from_doc(doc, fs=fs)
...
def get_service_context():
    """
    获取服务上下文，使用 Ollama 作为 LLM 和 embedding 模型
    """
    llm = OllamaLLM()
